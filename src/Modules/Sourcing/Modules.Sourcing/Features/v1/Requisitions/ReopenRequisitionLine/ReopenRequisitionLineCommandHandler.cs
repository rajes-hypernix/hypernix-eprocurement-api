...
public sealed class ReopenRequisitionLineCommandHandler(SourcingDbContext dbContext)
    : ICommandHandler<ReopenRequisitionLineCommand, Guid>
{
    public async ValueTask<Guid> Handle(ReopenRequisitionLineCommand command, CancellationToken cancellationToken)
    {
        ...
        pr.ReopenLine(command.LineId, DateTime.UtcNow);
