...
public sealed class UpdateEntryFormDetailsCommandHandler(PlatformDbContext dbContext)
    : ICommandHandler<UpdateEntryFormDetailsCommand, Guid>
{
    public async ValueTask<Guid> Handle(UpdateEntryFormDetailsCommand command, CancellationToken cancellationToken)
    {
        ...
        form.UpdateDetails(command.Name);
