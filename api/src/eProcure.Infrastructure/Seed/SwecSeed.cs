...
/// <summary>
/// The PETRONAS-style SWEC taxonomy, mirroring the prototype's SWEC_TREE
/// (Discipline → Group → Item). Flattened into <see cref="SwecCategory"/> rows.
/// </summary>
public static class SwecSeed
{
    ...
    public static List<SwecCategory> Build()
    {
        ...
        void Walk(Node[] nodes, string? parent, List<string> path)
        {
            foreach (var n in nodes)
            {
                ...
                var isLeaf = n.Children is null or { Length: 0 };
                rows.Add(new SwecCategory(n.Code, n.Name, parent, p.Count, isLeaf, string.Join(" › ", p)));
                if (!isLeaf) Walk(n.Children!, n.Code, p);
            }
        }
        Walk(Tree, null, []);
        return rows;
