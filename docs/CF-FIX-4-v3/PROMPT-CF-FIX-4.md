...
- **L2 — FLAT field groups, SIMPLIFIED (operator refined during testing):** multiple field groups per
  form, **NO parent/subgroup, no relationships between groups.** Just a flat list of named groups per
  form. This is SIMPLER than any tree — do not build nesting. (Confirmed by operator after testing:
  "just multiple field groups per form no relationships — easier architecture.")
...
**Operator-confirmed missing (finding #6, screenshot EF-new-pr):** on the Create Purchase Requisition
screen there is NO control to choose which entry form to use, even though multiple PR forms now exist.
On a transaction (PR, PO, GRN, Invoice) where **multiple forms exist** for that type, the user can
**choose which form** to use — a visible form picker (use the standardized `SearchSelectField`).
Respect the existing server rule (OD-D7-2: a chosen form can't dodge its role form's required fields —
the server re-resolves and enforces). If only one form exists, no picker (or a disabled indicator).
Default selection follows the existing role-precedence resolution. Browser test: with 2 PR forms, the
picker appears on New PR, switches layout, and the chosen form's fields/groups render.

---

## T3-FIX — editor defects found in operator testing (T3 committed but incomplete) — `CF-FIX4-T3fix:`

Operator tested the committed T3 layout designer and found six issues. These are FIXES/completions to
T3, done before continuing T4→T8. Screenshots in `reference-screenshots/` (EF-editor-* and the
New-PR screen).

1. **Add-field picker isn't the searchable select.** The "add field to form" dropdown is a plain
   `<select>`. Replace with the standardized typeable `SearchSelectField` (the same component locked
   for everywhere lists are picked — custom fields, lists, segments). Type-to-filter, full names.
2. **BUG — field shows internal-id instead of label once on the form.** In the editor, a placed field
   renders its `FieldKey` (`custbody_partner`, `seg_project_444214`) instead of its display label —
   even though the field's list shows the proper name. ROOT CAUSE (confirmed): the runtime resolver
   correctly uses `f.Label ?? reg.Label`, but the EDITOR's row display + add-field picker show the raw
   `FieldKey`. Fix: the editor must resolve the key to its label (`reg.Label`, honoring any `f.Label`
   override) exactly as `ResolveAsync` does — show the NAME, never the internal id, in both the picker
   options and the placed-field rows. (Internal id may show as secondary/muted text, but the primary
   display is the label.)
3. **Remove the Default-value setting entirely.** The `DEFAULT` column (`@today+7d …`) is not wanted.
   Remove the default-value control from the editor UI. (Leave the underlying `DefaultValue` column in
   the model harmless/unused, or drop it if clean — but the UI must not expose default-value editing.)
   Confirm in Step 0 whether removing the column needs a migration or is UI-only.
4. **Drag-drop between field groups doesn't work.** Creating a group works, but dragging a field from
   one group into another (e.g. `custbody_partner` → "Partners" group) does nothing. Fix the drag-drop
   so moving a field between groups updates its `GroupId` (the L1 placement object) and persists.
5. **Field groups simplified + move up/down arrows.** Per L2 (refined): flat groups, no parent/subgroup.
   AND add explicit **move up / move down arrow controls** on each field row (and each group) as a
   reliable alternative to drag — drag must also work (point 4), but arrows are the accessible/always-
   works fallback the operator wants. Moving a field up/down within or across groups updates its
   placement (group + sort).
6. (Form picker on the transaction is finding #6 → that's T5 below, not yet built.)

Browser tests: add-field picker is the searchable select and shows labels not ids; a placed field
shows its label; no default-value control exists; drag a field between two groups and it persists;
move-up/down arrows reorder and re-group correctly.
...
## SEQUENCING
T1/T2/T3 are already committed. **Do T3-FIX first** (the six testing defects — the editor must be
correct before T4's cascade writes into it and before segments reuse it). Then the confirmed T4
placement cascade, then the rest. Order:
**T3-FIX (editor defects) → T4 (creation cascade, placement schema confirmed) → T5 (transaction form
picker) → T6 (segment lists/lifecycle) → T7 (segment placement) → T8 (Archive tier).**
The T4 placement design is CONFIRMED (see the confirmation note): EntryFormField is the SOLE placement
authority; CustomFieldDefApplication holds applies-to-record-type + a default-group hint only; the
cascade CREATES EntryFormField rows; option (c) form-picking with the standard form pre-selected; the
reconciliation invariant is BIDIRECTIONAL (applies-to ⟺ placement rows never disagree); removing the
last placement on a form-type updates/drops applies-to.
