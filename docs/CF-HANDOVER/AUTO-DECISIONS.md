...
## AD-5 · Keep the VIEW-DEMO example saved views in the seed (T5)
- **Decision:** `SeedExampleViewsAsync` (VIEW-DEMO-0001…0005 — "PRs pending approval", "Draft requisitions", "RFQs closing this month", "POs awaiting acknowledgement", "Invoices in exception") STAYS in the seed. T2's ledger box called VIEW-DEMO "test noise", but these are realistic, useful example views.
- **Why:** (a) they are realistic operator starting points that populate the view/reminder pickers — CF3-T11 added them precisely to fix the "reminders don't work / near-empty picker" root cause; removing them regresses that. (b) 10-cf-parity depends on "PRs pending approval" existing (picks it in the Saved-view picker, asserts the reminder row + KPI). The dev-DB litter T2 removed was the ACCUMULATED per-run test views + the "Requisition with Partnets" typo view + personal views — not these curated shared examples.
- **Reverse:** delete the `SeedExampleViewsAsync` call in SeedAsync (and update 10-cf-parity).

## AD-6 · PR-dimension convergence deferred — segments carry curated 5 + demo-PR dims (T5)
- **Decision:** the curated segments (Department/Location/Category/Job/Project) each get their 5 curated values from `SeedHandoverConfigAsync`, but the live value set is LARGER (Dept/Loc/Cat ≈7, Job ≈14) because `ProjectPrSegmentsAsync` projects every seed PR's dimension into its segment — and the rich P2P demo PRs (SourcingSeed, PR-2026-04xx) use legacy dims (Facilities, "Valves & Fittings", JOB-CWS-014, OPEX-2026, …) that are realistic but off the curated list.
- **Why:** aligning the 04xx demo PRs' dims to the curated set would touch many seed records AND risks the 31 e2e files that assert on those PRs' specific values. The handover value (curated pickers + clean field/list/segment/view set + Item Master + 10 curated PRs) does not require exactly-5. The 10 new CFH-T5 PRs (PR-2026-09xx) use ONLY curated dims + Item-Master item codes, so they add no new projected litter. This is the standing "PR dimension convergence" deferral.
- **Reverse / follow-up:** rewrite SourcingSeed's PR/RFQ/PO dims to the curated vocabulary, then re-baseline the dependent e2e assertions.
