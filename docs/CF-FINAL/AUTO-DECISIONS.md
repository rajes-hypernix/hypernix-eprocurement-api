...
## AD-7 · T9 cleanup — bulk deleted, value-bearing residue governed-inactivated
- **Decision:** deleted the bulk dummy litter across the dev DB — ~65 zero-value custom fields (all "Budget Ref"/"Site Ref"/Fix* zero-value), 9 litter custom lists (Fix1 Values/Fix3 List/T10 Tree), 44 stamped segments (Project NNN/Fix4 Seg), and 34 litter saved views. Curated realistic config kept/created: fields Cost Centre + Partner + Remarks; lists Bank/City/Country/Currency/Customer/Partner List/Payment terms/Plant/RFQ reasons/State; segments Project Code Grouped + the 4 system dimensions. ~82 value-bearing litter fields (mostly repeated "Warranty Expiry" from 06-custom-fields runs, each with one stray value on an open record) are DEACTIVATED, not deleted.
- **Why:** the data-safety guard (L6/CF-FIX-3) refuses to delete or purge a field with a live value on an OPEN record — "a field in live use is never deleted." The record endpoints don't surface those particular stray values for clearing (out-of-scope/paginated), so the governed outcome is to INACTIVATE them — exactly what the ledger sanctions ("inactivate where records reference them — never orphan"). The ACTIVE config is clean and realistic; retired litter is badged Deactivated.
- **Reverse:** re-create the deleted litter (there is no reason to). To further reduce the inactive residue, clear each field's stray value on its owning record (requires locating the record) then delete.
