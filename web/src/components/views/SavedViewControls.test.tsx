...
describe('ViewBuilder', () => {
  it('builds and saves the gate criterion — Status is Open AND ClosesUtc between month tokens', async () => {
    ...
    // T7: Field + Operator are searchable SearchSelectFields (button → option), not native selects.
    await userEvent.click(screen.getByLabelText('Operator'))
    await userEvent.click(await screen.findByRole('option', { name: 'Eq' }))
    ...
    const fieldButtons = screen.getAllByLabelText('Field')
    await userEvent.click(fieldButtons[1])
    await userEvent.click(await screen.findByRole('option', { name: 'Closes' }))
    const opButtons = screen.getAllByLabelText('Operator')
    await userEvent.click(opButtons[1])
