    ...
    nodes.filter(matches).map((n) => (
      <div key={n.code}>
        <label className="swnode" style={{ paddingLeft: depth * 18 }}>
          ...
          {n.isLeaf && <span className="hint" style={{ marginLeft: 'auto' }}>{n.code}</span>}
        </label>
    ...
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="modal">
        ...
        <div className="mbody">
          ...
          <div className="swtree">
            {swec ? renderNodes(swec.tree, 0) : <Spinner />}
          </div>
          <div style={{ marginTop: 12 }}>
            ...
            <div>
              ...
              {sel.size === 0 && <span className="hint">None selected — tick categories above.</span>}
            </div>
          </div>
        </div>
        <div className="mfoot">
          ...
          <button type="button" className="btn btn-pri" onClick={() => onSave([...sel])}>
            <Icon name="chev" size={15} /> Save categories
          </button>
        </div>
      </div>
    </div>
