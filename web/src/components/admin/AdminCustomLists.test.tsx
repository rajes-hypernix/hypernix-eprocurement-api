...
describe('AdminCustomLists (NetSuite-style custom lists)', () => {
  ...
  it('adds a value to the selected list (stores code + label)', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Value code'), 'NET90')
    await userEvent.type(screen.getByLabelText('Value label'), '90 days')
  ...
  it('a dependent list exposes the parent-value picker', async () => {
    ...
    expect(await screen.findByLabelText('Parent value')).toBeInTheDocument()
  ...
  it('creates a new list', async () => {
    ...
    await userEvent.type(screen.getByLabelText('List code'), 'incoterm')
    await userEvent.type(screen.getByLabelText('List name'), 'Incoterms')
